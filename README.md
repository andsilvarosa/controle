...
&#x20; <img width="1648" height="640" alt="Image" src="https://github.com/user-attachments/assets/aa1eceb3-99c7-4220-8fcf-8dcce681bf07" />
...
│   budgets, categories, rules, audit\_logs, etc.              │
...
JWT\_SECRET=sua\_chave\_secreta\_aqui

RESEND\_API\_KEY=re\_sua\_chave\_aqui

APP\_URL=http://localhost:5173

ADMIN\_SECRET=senha\_admin\_para\_setup\_db
...
| `DATABASE\_URL` | String de conexão com o banco Neon PostgreSQL |

| `JWT\_SECRET` | Chave secreta para assinatura dos tokens JWT |

| `RESEND\_API\_KEY` | Chave da API Resend para envio de e-mails |

| `APP\_URL` | URL base da aplicação |

| `ADMIN\_SECRET` | Senha para acessar o endpoint de inicialização do banco |
...
Após configurar as variáveis, acesse o endpoint de setup (protegido por `ADMIN\_SECRET`):
...
&#x20; -d '{"adminSecret": "sua\_senha\_admin"}'
...
| `known\_devices` | Rastreamento de dispositivos conhecidos |

| `audit\_logs` | Log de auditoria de todas as ações |

| `password\_resets` | Tokens de recuperação de senha |

| `rate\_limits` | Armazenamento de rate limiting |
...
| `recurrence\_exceptions` | Exceções de datas para transações recorrentes |
